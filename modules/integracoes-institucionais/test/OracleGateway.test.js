...
describe("OracleGateway", function () {
  ...
  describe("recordEvent", function () {
    it("operador deve registrar evento com sucesso", async function () {
      const tx = await oracleGateway
        .connect(operator)
        .recordEvent(EVENT_DEATH, SUBJECT_ID, DATA_HASH, SIG_HASH, INSTITUTION);
    ...
    it("deve rejeitar subjectId vazio", async function () {
      await expect(
        oracleGateway
          .connect(operator)
          .recordEvent(EVENT_DEATH, ethers.ZeroHash, DATA_HASH, SIG_HASH, INSTITUTION),
  ...
  describe("WhiteLabelRegistry guard", function () {
    it("sem registry configurado deve aceitar qualquer institution", async function () {
      ...
      await expect(
        oracleGateway
          .connect(operator)
          .recordEvent(EVENT_DEATH, SUBJECT_ID, DATA_HASH, SIG_HASH, "QUALQUER_PARCEIRO"),
      ).to.not.be.reverted;
    });
  });
});
...
describe("WhiteLabelRegistry", function () {
  ...
  it("deve rejeitar duplicata de partnerCode", async function () {
    ...
    await registry
      .connect(registrar)
      .registerPartner("DUPLICATE", "Parceiro", 0, 0, contractHash, partnerWallet.address, "BR");
    await expect(
      registry
        .connect(registrar)
        .registerPartner("DUPLICATE", "Outro", 0, 0, contractHash, partnerWallet.address, "BR"),
    ).to.be.reverted;
  });
});
