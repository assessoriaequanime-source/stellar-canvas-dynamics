...
describe("TimeCapsule", function () {
  ...
  beforeEach(async function () {
    ...
    await sglToken
      .connect(creator)
      .approve(await timeCapsule.getAddress(), ethers.parseEther("1000"));
  });
  ...
  describe("cancelCapsule", function () {
    ...
    it("deve rejeitar cancelamento por não-criador", async function () {
      ...
      await expect(timeCapsule.connect(beneficiary).cancelCapsule(0)).to.be.revertedWith(
        "CAPSULE: not creator",
      );
    });
  });
  ...
  describe("ManualCurator flow", function () {
    ...
    it("deve rejeitar executor sem aprovação do curador", async function () {
      ...
      await expect(timeCapsule.connect(executor).executeApprovedCapsule(0)).to.be.revertedWith(
        "CAPSULE: curator not approved",
      );
    });
  });
  ...
  describe("LegacyPolicy", function () {
    ...
    it("deve ativar e executar política com POLICY_MANAGER_ROLE", async function () {
      await legacyPolicy
        .connect(creator)
        .createPolicy("QmOk", [beneficiary.address], [10000], ["herdeiro"]);
