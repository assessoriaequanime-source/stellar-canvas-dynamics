...
import {
  ...
  loadAuditRecords,
  type AuditRecord,
} from "@/lib/sgl/execution";
import {
  INITIAL_SGL_BALANCE,
  SERVICE_CATALOG,
  SERVICE_TYPES,
  type ServiceType,
} from "@/lib/sgl/services";
    ...
    <div style={{ height: "100dvh", overflowY: "auto", background: "#0b0b0b" }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 16px", color: "#f7f5ef" }}>
        ...
        <p style={{ opacity: 0.9, marginBottom: 20 }}>
          Operational MVP for paid execution services with Solana demo audit proofs.
        </p>
        ...
        <section style={{ ...CARD, marginBottom: 16 }}>
          ...
          <p>
            Private content is never stored on-chain. Only hashes, proofs and execution states are
            public.
          </p>
        </section>
        ...
        >
          <div style={CARD}>
            ...
            <p style={{ fontSize: 10, opacity: 0.5, marginBottom: 8, wordBreak: "break-all" }}>
              {walletAddress}
            </p>
            <button style={BUTTON} onClick={() => copyValue(walletAddress, "wallet")}>
              Copy wallet
            </button>
          </div>
          <div style={CARD}>
          ...
          >
            {SERVICE_TYPES.map((serviceType) => (
              <button
                ...
                disabled={isRunning}
                style={BUTTON}
                onClick={() => handleExecute(serviceType)}
              >
        ...
        {latest ? (
          <section style={{ ...CARD, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              <button style={BUTTON} onClick={() => copyValue(latest.txSignature, "txSignature")}>
                Copy txSignature
              </button>
              <button style={BUTTON} onClick={() => copyValue(latest.payloadHash, "payloadHash")}>
                Copy payloadHash
              </button>
              <button style={BUTTON} onClick={() => copyValue(latest.snapshotHash, "snapshotHash")}>
                Copy snapshotHash
              </button>
              <button style={BUTTON} onClick={() => copyValue(latest.explorerUrl, "explorerUrl")}>
                Copy explorerUrl
              </button>
            </div>
          </section>
            ...
            {records.map((record, index) => (
              ...
              >
                <p style={{ margin: "0 0 6px" }}>
                  <strong>{record.serviceType}</strong> - {record.cost} SGL
                </p>
                <p style={{ margin: "0 0 6px" }}>
                  Balance: {record.previousBalance} to {record.newBalance}
                </p>
                <p style={{ margin: "0 0 6px" }}>Status: {record.status}</p>
                <p style={{ margin: "0 0 6px", wordBreak: "break-all" }}>
                  tx: {record.txSignature}
                </p>
                <a
        ...
        <p style={{ marginTop: 6, opacity: 0.8 }}>
          Initial SGL balance for this MVP is fixed at {INITIAL_SGL_BALANCE.toLocaleString()}.
        </p>
        <footer
          style={{
            marginTop: 40,
