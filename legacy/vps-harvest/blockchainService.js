  ...
  initialize() {
    ...
    if (privateKey) {
      try {
        ...
        for (const [name, config] of Object.entries(CONTRACTS)) {
          this.signedContracts[name] = new ethers.Contract(config.address, config.abi, this.wallet);
        }
  ...
  async registerOnChain(type, params, retries = 2) {
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        console.error(
          `${type} on-chain registration error (attempt ${attempt + 1}):`,
          error.message,
        );
        if (
          attempt < retries &&
          (error.message.includes("nonce") || error.message.includes("replacement"))
        ) {
