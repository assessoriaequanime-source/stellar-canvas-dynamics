...
class WalletService {
  ...
  initialize() {
    try {
      ...
      if (distributorPrivateKey) {
        ...
        this.sglContract = new ethers.Contract(
          SGL_TOKEN_ADDRESS,
          ERC20_ABI,
        ...
        console.warn(
          "Wallet service: SGL_DISTRIBUTOR_PRIVATE_KEY not configured - using simulation mode",
        );
      }
    } catch (error) {
  ...
  async createUserWallet(userId, options = { distributeTokens: true }) {
    ...
    return {
      ...
      gasEthTransactionUrl: gasResult?.hash
        ? `https://sepolia.etherscan.io/tx/${gasResult.hash}`
        : null,
  ...
  async transferSGL(fromAddress, amount, reason = "transfer") {
    ...
    if (currentBalance < amount) {
      throw new Error(
        `Saldo insuficiente: ${currentBalance} SGL disponível, ${amount} SGL necessário`,
      );
    }
  ...
  encryptKey(key) {
    ...
    const secret =
      process.env.WALLET_ENCRYPTION_KEY || "singulai-default-key-change-in-production-32";
