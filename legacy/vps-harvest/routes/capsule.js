...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const receivedCapsules = user?.email
      ? await storageService.getCapsulesByRecipient(user.email)
      : [];
  ...
  try {
    ...
    if (!senderId || !senderWallet || !recipientEmail || !title || !message || !deliveryType) {
      return res.status(400).json({
        error:
          "Campos obrigatórios: senderId, senderWallet, recipientEmail, title, message, deliveryType",
      });
    }
    ...
    let txResult;
    try {
      txResult = await walletService.transferSGL(
        senderWallet,
        SGL_COST_PER_CAPSULE,
        "capsule_creation",
      );
    ...
    if (
      ...
      sender.walletAddress.toLowerCase() !== senderWallet.toLowerCase()
    ) {
    ...
    const recipientAddress =
      existingUser?.walletAddress || "0x0000000000000000000000000000000000000000";
    ...
    try {
      ...
      console.log(
        `Parameters: avatarId=${senderAvatarId}, contentHash=${contentHash.substring(0, 16)}..., unlockTime=${unlockTime}, recipient=${recipientAddress}`,
      );
      ...
      if (blockchainResult?.hash) {
        ...
        console.log(
          `Capsule registered on blockchain: ${blockchainTxHash} (status: ${blockchainStatus})`,
        );
      }
    ...
    if (deliveryType === "immediate" && tempWallet) {
      ...
      console.log(
        `Immediate capsule email sent to ${recipientEmail} with access code ${tempWallet.accessCode}`,
      );
    }
    ...
    console.log(
      `Capsule created successfully: ${capsuleId}, blockchain status: ${blockchainStatus}`,
    );
