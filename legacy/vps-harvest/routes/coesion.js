...
router.post("/calculate", (req, res) => {
  try {
    ...
    const omega = coesionService.calculateOmega(particles, interactivity, absorption || 0.5);
...
router.get("/thresholds", (req, res) => {
  res.json({
    ...
    thresholds: {
      ...
      STABLE: { min: 0.8, description: "Avatar estável com boa coesão" },
      DEVELOPING: { min: 0.6, description: "Avatar em desenvolvimento" },
      EMERGING: { min: 0.4, description: "Avatar emergente" },
      INITIALIZING: { min: 0.0, description: "Avatar em inicialização" },
    },
    formula: {
