...
router.post("/login", async (req, res) => {
  try {
    ...
    if (!user.passwordHash) {
      return res
        .status(401)
        .json({ error: 'Esta conta usa login Google. Clique em "Entrar com Google".' });
    }
          ...
          if (bonusResult.success) {
            ...
            if (bonusResult.txHash) {
              await storageService.saveTransaction({
                ...
                blockNumber: null,
                from:
                  process.env.SGL_DISTRIBUTOR_ADDRESS ||
                  "0x3d3C2E249f9F94e7cfAFC5430f07223ec10AD3bb",
                to: user.walletAddress,
                amount: INITIAL_SGL_BONUS,
                type: "initial_distribution",
              });
...
router.get("/google/callback", async (req, res) => {
  try {
    ...
    const redirectUrl =
      ...
      "&user=" +
      encodeURIComponent(JSON.stringify(redirectUser)) +
      "&wallet=" +
      encodeURIComponent(JSON.stringify({ address: redirectUser.walletAddress }));
