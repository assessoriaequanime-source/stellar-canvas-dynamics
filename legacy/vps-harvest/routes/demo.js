...
router.post("/chat", async (req, res) => {
  try {
    ...
    if (!consentToken || typeof consentToken !== "string") {
      return res
        .status(403)
        .json({ success: false, error: "Consent required before using demo chat" });
    }
    const tokenData = consentTokens.get(consentToken);
    if (!tokenData) {
      return res.status(403).json({
        success: false,
        error: "Invalid or expired consent token. Please accept the consent again.",
      });
    }
...
router.post("/consent", (req, res) => {
  try {
    ...
    const filteredRegulations = regulations.filter(
      (r) => typeof r === "string" && validRegulations.includes(r),
    );

    const validConsents = ["ip_tracking", "session_data", "usage_analytics"];
    const filteredConsent = consent.filter(
      (c) => typeof c === "string" && validConsents.includes(c),
    );
    ...
    console.log(
      `[CONSENT:ACCEPTED] IP: ${clientIP} | Regulations: ${filteredRegulations.join(", ")} | Lang: ${language}`,
    );
