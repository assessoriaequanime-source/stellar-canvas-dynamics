...
router.post("/avatars/create", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: tx.verified
        ? "Avatar criado na blockchain com sucesso"
        : "Avatar criado (simulação)",
      transaction: tx,
    });
  } catch (error) {
...
router.post("/wallet/link", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: tx.verified
        ? "Wallet vinculada ao avatar na blockchain"
        : "Wallet vinculada (simulação)",
      transaction: tx,
    });
  } catch (error) {
...
router.post("/capsules/create", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: tx.verified
        ? "Cápsula criada na blockchain com sucesso"
        : "Cápsula criada (simulação)",
      transaction: tx,
    });
  } catch (error) {
  ...
  try {
    ...
    if (data.items && Array.isArray(data.items)) {
      ...
      res.json({
        ...
        transactions: [],
        total: 0,
        note: "Nenhuma transação encontrada",
      });
    }
  } catch (error) {
