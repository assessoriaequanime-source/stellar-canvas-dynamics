...
const GOOGLE_REDIRECT_URI =
  process.env.GOOGLE_REDIRECT_URI || `${PUBLIC_API_BASE}/auth/google/callback`;
...
async function ensureWallet(client, user) {
  const existing = await client.query(`SELECT * FROM wallets WHERE user_id = $1 LIMIT 1`, [
    user.id,
  ]);
  ...
  await client.query(`UPDATE users SET wallet_address = $1, updated_at = now() WHERE id = $2`, [
    address,
    user.id,
  ]);
...
async function ledger(client, user, amount, type, reason, metadata = {}) {
  ...
  await client.query(`UPDATE users SET sgl_balance = $1, updated_at = now() WHERE id = $2`, [
    after,
    user.id,
  ]);
  ...
  const entry = await client.query(
    `INSERT INTO sgl_ledger
     ...
     RETURNING *`,
    [randomId("ledger"), user.id, amount, type, reason, before, after, JSON.stringify(metadata)],
  );
...
async function firstBonus(client, user) {
  ...
  await client.query(`UPDATE users SET first_bonus_sent = true, updated_at = now() WHERE id = $1`, [
    user.id,
  ]);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    await pool.query(`INSERT INTO oauth_states (state, redirect_to) VALUES ($1, $2)`, [
      state,
      redirectTo,
    ]);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const wallet = (await pool.query(`SELECT * FROM wallets WHERE user_id = $1 LIMIT 1`, [userId]))
      .rows[0];
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      const debit = await charge(client, req.user, "avatar_message", {
        modelId,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    const result = await pool.query(
      `SELECT * FROM legacies WHERE user_id = $1 ORDER BY created_at DESC`,
      [req.user.id],
    );
      ...
      const result = await client.query(
       ...
       RETURNING *`,
        [
          ...
          type,
          JSON.stringify({
            message: `Ação ${type} registrada no backend alternativo.`,
            etaSeconds: 15,
          }),
